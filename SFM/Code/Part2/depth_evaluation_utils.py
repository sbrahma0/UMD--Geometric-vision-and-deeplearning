...
#         camera_id = filename[-1]   # 2 is left, 3 is right
	date = filename[0:10]
        ...
        file_root = '{}/{}'
        im = filename
        vel = '{}/{}/velodyne_points/data/{}.bin'.format(filename[0:10], filename[:26], im_id)
	
        if os.path.isfile(data_root + "/" + filename[:29] + "/" + im_id + ".jpg"):
	    
            gt_files.append(data_root + vel)
            gt_calib.append(data_root + "/" + date + '/')
            im_sizes.append(cv2.imread(data_root + "/" + filename[:29] + "/" + im_id + ".jpg").shape[:2])
            im_files.append(data_root + im)
            cams.append(2)
        else:
            num_probs += 1
            print('{} missing'.format(data_root + "/" + filename[:29] + "/" + im_id))
...
def generate_depth_map(calib_dir, velo_file_name, im_shape, cam=2, interp=False, vel_depth=False):
    print(calib_dir, velo_file_name, im_shape)
    exit()
