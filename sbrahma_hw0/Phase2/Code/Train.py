...
from Network.Network import CIFAR10Model, ResNetModel, ResNext, DenseNet
...
def GenerateBatch(BasePath, DirNamesTrain, TrainLabels, ImageSize, MiniBatchSize):
	...
	while ImageNum < MiniBatchSize:
		...
		I1noise = np.float32(random_noise(cv2.imread(RandImageName), mode='gaussian', var=0.01))
...
def TrainOperation(ImgPH, LabelPH, DirNamesTrain, TrainLabels, NumTrainSamples, ImageSize,
    ...
    prLogits, prSoftMax = CIFAR10Model(ImgPH, ImageSize, MiniBatchSize*3) #CNN
    #prLogits, prSoftMax = ResNetModel(ImgPH, ImageSize, MiniBatchSize*3) #Residual
    #prLogits, prSoftMax = ResNext(ImgPH, ImageSize, MiniBatchSize*3) #ResNext
    #prLogits, prSoftMax = DenseNet(ImgPH, ImageSize, MiniBatchSize*3) #DenseNet
 
    with tf.name_scope('Loss'):
        ###############################################
        # Fill your loss function of choice here!
        ###############################################
        loss = tf.compat.v1.losses.softmax_cross_entropy(LabelPH, prLogits)
...
def main():
    ...
    Parser.add_argument('--BasePath', default='/home/sayan/PycharmProjects/cmsc733/Phase2/CIFAR10', help='Base path of images, Default:/media/nitin/Research/Homing/SpectralCompression/CIFAR10')
    ...
    ImgPH = tf.compat.v1.placeholder(tf.float32, shape=(MiniBatchSize*3, ImageSize[0], ImageSize[1], ImageSize[2]))
    LabelPH = tf.compat.v1.placeholder(tf.float32, shape=(MiniBatchSize*3, NumClasses)) # OneHOT labels
