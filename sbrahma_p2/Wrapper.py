#!/usr/bin/env python
# Authors : Nikhil Mehra and Sayan Brahma
# CMSC-733 Project-2(Face Swap)
...
import copy
from delaunay_triangulation import swap_faces
...
def main():
	...
	predictor = dlib.shape_predictor('/home/sayan/Downloads/CMSC-733-master/Abhi1625_p2/shape_predictor_68_face_landmarks.dat')
    # /home/pratique/Downloads/cmsc733-Pictorial Information/project_2_face_swap/YourDirectoryID_p2
    #Setup input video
    # Add any Command Line arguments here
	Parser = argparse.ArgumentParser()
	Parser.add_argument('--video', default='Test3.mp4', help='Provide Video Name here')
	Parser.add_argument('--swap_img',default = 'Scarlett.jpg',help='Provide Img to be swapped in the image.')
	Parser.add_argument('--mode',default = 'tps',help='Provide Method of image transformation: delaunay(TRI), TPS(tps)')
	...
	img_tar = cv2.imread(swap_img)
	i=0
	while(True):
		ret1,img_src = cap1.read()
		if ret1 == False:
			break
		#cv2.imwrite('org'+str(i)+'.jpg',img_src)
		...
		if(flag1==False or flag2==False):
			...
			continue
			#Check which mode is called delaunay or TPS
		if mode=='tps':
		#TRANSFORM THE IMAGE USING TPS
			...
			#TRANSFORM THE IMAGE USING DELN
			print(img_source.shape)
			output1, flag = swap_faces(img_target.copy(), img_source.copy(), detector,predictor)
			print(flag)
			output1 = cv2.resize(output1,((rects[0].right()+40)- (rects[0].left()-40),(rects[0].bottom()+40)-(rects[0].top()-40) ))
			if(flag==False):
				cv2.imwrite(+str(i)+'.jpg',img_src)
			...
			img_src[rects[0].top()-40:rects[0].bottom()+40,rects[0].left()-40:rects[0].right()+40,:] = output1
			if len(rects)>1:
				output2, flag = swap_faces(img_source.copy(), img_target.copy(), detector,predictor)
				print(flag)
				output2 = cv2.resize(output2,((rects[1].right()+40)- (rects[1].left()-40),(rects[1].bottom()+40)-(rects[1].top()-40) ))
				img_src[rects[1].top()-40:rects[1].bottom()+40,rects[1].left()-40:rects[1].right()+40,:] = output2
		#Write output frame
